...
try {
  const testPathPatterns = filesContainingTag.map(file => `"${file.replace(/\\/g, '\\\\')}"`).join('|')
  const testNamePattern = tag
  const command = `jest --testPathPatterns="${testPathPatterns}" --testNamePattern="${testNamePattern}"`
