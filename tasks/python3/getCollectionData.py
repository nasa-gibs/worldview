...
def get_cmr_data(concept_id_dict):
  ...
  if len(entry) == 1:
    ...
    concept_id_dict = dict_merge(concept_id_dict, cmr_entry_dict)
  elif len(entry) > 1:
...
def process_requests(wv_product_dict):
  ...
  print("%s: Finished processing requests." % (prog))

  # collection_data = cmr_data #dict_merge(cmr_data, cmr_umm_data)
  # no_data_count = 0
