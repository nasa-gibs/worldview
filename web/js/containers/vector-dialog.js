...
function VectorDialog(props) {
  ...
  function updateIndex(activeIndexIn) {
    if (activeIndexIn === activeIndex) return;
  ...
  for (const [key, value] of Object.entries(vectorMetaObject)) {
    ...
    navArray.push(
      ...
      >
        <NavLink onClick={() => updateIndex(index)} title={combinedTitles}>
