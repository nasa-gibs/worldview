...
function VectorMetaTable({ id, metaArray, title }) {
  return (
    <div>
...
export default React.memo(VectorMetaTable, (prevProps, nextProps) => {
  if (
    prevProps.id &&
    prevProps.title && nextProps.id && nextProps.title &&
    prevProps.id === nextProps.id && prevProps.title === nextProps.title) {
    return true;
  }
  return false;
});
