...
import WelcomeCustom from './welcome-custom';
import KeyboardCustom from './keyboard-custom';
import ImageryCustom from './imagery-custom';
import AcknowledgementsCustom from './acknowledgments-custom';
import DisclaimerCustom from './disclaimer-custom';
import LicenseCustom from './license-custom';
  ...
  return (
    <Scrollbar style={{ height: 'calc(var(--vh, 1vh) * 100 - 100px)' }}>
      <div className="about-page">
        {overrideWelcome ? <WelcomeCustom /> : <Welcome />}
        {overrideKeyboardShortcuts ? <KeyboardCustom /> : <KeyboardShortcuts />}
        {overrideImageryUse ? <ImageryCustom /> : <Imagery />}
        {overrideAcknowledgments ? <AcknowledgementsCustom /> : <Acknowledgements />}
        {overrideDisclaimer ? <DisclaimerCustom /> : <Disclaimer />}
        {overrideLicense ? <LicenseCustom /> : <License />}
        {overrideFooter ? <FooterCustom /> : <Footer />}
      </div>
    </Scrollbar>
  );
}
