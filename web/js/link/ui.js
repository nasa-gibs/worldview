...
export function linkUi(models, config) {
  ...
  self.show = function() {
    ...
    if (config.features.urlShortening) {
      item +=
        "<span autofocus></span><div id='wv-link-shorten'>" +
    ...
    var Widget = React.createElement(Share, {
    ...
    var error = function() {
      ...
      wvui.notify(
        'Unable to shorten the permalink at this time. ' +
    ...
    clipboard.on('success', function(e) {
      ...
      $('.wv-link .input-group-button').removeClass('hide-icon');

      setTimeout(function() {
        $('.wv-link .input-group-button').addClass('hide-icon');
      }, 1000);
    });
