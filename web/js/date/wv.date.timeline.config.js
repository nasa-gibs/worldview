...
wv.date.timeline.config = wv.date.timeline.config || function(models, config, ui) {
    ...
    self.level1 = function(){
        var altEnd, fNormData;
        //format of the label
        var labelFormat = d3.time.format.utc("%Y");
        //printed type of tick
        var dateInterval = d3.time.year.utc;
        //step of the ticks
        var dateStep = 1;
        //number of ticks total of data range, in days
        var tickCount = tl.data.end().getUTCFullYear() - tl.data.start().getUTCFullYear();
    ...
    self.level2 = function(){
        ...
        var labelFormat = d3.time.format.utc("%Y");
        //printed type of tick
        var dateInterval = d3.time.month.utc;
        //step of the ticks
        var dateStep = 1;
        //number of ticks total of data range, in days
        var tickCount = (tl.data.end().getUTCFullYear() - tl.data.start().getUTCFullYear()) * 12 +
            tl.data.end().getUTCMonth() + 1 - tl.data.start().getUTCMonth();
        ...
        //Value for hovered boundary tick
        tl.zoom.current.ticks.boundary.hover = function(d){
            return new Date(d.getUTCFullYear(),model.selected.getUTCMonth(),model.selected.getUTCDate());
        };
        ...
        //When the date updates while dragging the pick forward
        tl.zoom.current.pick.nextChange = function(d){
            return new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth()+1,model.selected.getUTCDate()));
        };

        //When the date updates while dragging the pick backward
        tl.zoom.current.pick.prevChange = function(d){
            return new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),model.selected.getUTCDate()));
        };
