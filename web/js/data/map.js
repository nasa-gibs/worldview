...
import { mapToPolys, mapDistanceX } from '../map/map';
...
export function dataMap(model, maps, config) {
  ...
  var buttonStyle = function (feature) {
    ...
    return [new OlStyleStyle({
      image: new OlStyleIcon({ src: image, scale: dimensions.scale })
    })];
  };
  ...
  var hoverStyle = function (feature) {
    ...
    if (!model.isSelected(feature.granule)) {
      return [new OlStyleStyle({
        fill: new OlStyleFill({ color: 'rgba(181, 158, 50, 0.25)' }),
        stroke: new OlStyleStroke({
      ...
      return [new OlStyleStyle({
        fill: new OlStyleFill({ color: 'rgba(242, 12, 12, 0.25)' }),
        stroke: new OlStyleStroke({
          color: 'rgb(255, 6, 0)',
          width: 3
        }),
  ...
  var createButtonLayer = function () {
    buttonLayer = new OlLayerVector({
      source: new OlSourceVector({ wrapX: false }),
      style: buttonStyle
    });
  ...
  var createSelectionLayer = function () {
    selectionLayer = new OlLayerVector({
      ...
      style: new OlStyleStyle({
        fill: new OlStyleFill({ color: 'rgba(127, 127, 127, 0.2)' }),
        stroke: new OlStyleStroke({
  ...
  var updateButtons = function () {
    ...
    lodashEach(granules, function (granule) {
      if (!granule.centroid || !granule.centroid[model.crs]) return;
      var centroid = granule.centroid[model.crs];
      var feature = new OlFeature({ geometry: centroid });
      feature.button = true;
  ...
  var updateSwaths = function () {
    ...
    if (!swaths) return;
    var maxDistance = (model.crs === 'EPSG:4326') ? 270 : Number.POSITIVE_INFINITY;
    var features = [];
  ...
  var hoverCheck = function (event) {
    ...
    map.forEachFeatureAtPixel(pixel, function (feature, layer) {
      if (feature.button) newFeature = feature;
    });
    if (hovering) hovering.changed();
    if (newFeature) newFeature.changed();
  ...
  var getButtonDimensions = function () {
    ...
    var base = 12; // Minimum size of the button is 12 pixels
    var add = Math.pow(2, zoom); // Double the size for each zoom level
    var size = Math.min(base + add, base + 32);
    return { scale: size / 48, size: size };
  };
