The MODIS Near Real-Time (NRT) Global Flood Product (MCDWD) provides a daily global map of flooding. It is derived from the NRT MODIS Surface Reflectance (MOD09) datasets from both the Terra and Aqua satellites. The Flood Product is available for 3 compositing periods: 1-day, 2-day, and 3-day. For each composite, water detections for all observations (Terra and Aqua) over the compositing period (1, 2, or 3 days) are accumulated, and if the total exceeds the required threshold, the pixel is marked as water. If these water pixels do not match known/expected water from a reference layer, they are labelled as “flood” in the product.

On December 11, 2025, the product was updated to Release 1.1, which includes a new “recurring flood” class. This helps discriminate unusual flooding from recurring flooding, which is defined as flooding which has been observed in the same location in roughly 1/3 of the years in the reprocessed 22 year archive (2003-2024). If identified water pixels match the recurring flood mask, they are labelled as “recurring flood” in the product (please see [User Guide](https://www.earthdata.nasa.gov/data/instruments/viirs/near-real-time-data/nrt-global-flood-products) for details).

Note: The 1-day product not available in Worldview, but can be viewed in the [Flood Location Observation and Overview Dashboard (FLOOD) Map](https://lance.modaps.eosdis.nasa.gov/flood/),select "Advanced Mode".
