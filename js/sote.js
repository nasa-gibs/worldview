...
function showOverlay(){
	if(this.overlay === undefined){
		this.overlay = new YAHOO.widget.Panel("panel1", {  visible:false } );
		var item = 	"<div >"+
			"<h3>Welcome to Worldview</h3>"+
			"<p>This new tool from EOSDIS provides the capability to interactively browse full-resolution, global, near real-time spacecraft imagery from 3 (eventually 30+) data products from LANCE <http://lance.nasa.gov> . This will support time-critical application areas such as wildfire management, air quality measurements, and weather forecasting. The data is generally available within three hours of observation and can be compared to observations from the past week. The interface also supports comparisons amongst available data products and time steps for the currently-zoomed region, facilitating a rapid visual filtering and selection of the most suitable product and time step. Browsing on tablet devices is currently supported for mobile access to this spacecraft imagery while in the field or on the couch. Please note, however, that this is an alpha release and many features are still under development!  In addition, please note that Internet Explorer is not currently supported and we are working to determine if and when it can be.  Comments/suggestions/problem reports are welcome via this address: <a href='mailto:eosdis-usersupport@lists.nasa.gov?subject=Feedback%20for%20EOSDIS%20Worldview%20tool'>eosdis-usersupport@lists.nasa.gov</a>.</p>"+ 
			"<br /><p>For more functionality and support for many more data products, please use the <a href='http://lance-modis.eosdis.nasa.gov/wms/'>LANCE WMS client</a> or, for a more hazards-and-disasters-specific experience, this <a href='http://lancedev.eosdis.nasa.gov/tools/hazards-and-disasters/'>introduction</a> to the LANCE WMS client.  As we move forward, these data products and additional functionality will be incorporated into Worldview and its Tiled WMS backend.</p>"+
			"<h3>Disclaimer</h3>"+
