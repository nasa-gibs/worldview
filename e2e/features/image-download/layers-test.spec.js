...
test.fixme('List layers in draw order', async () => {
...
test.fixme('Move AOD over the reference features', async ({ browserName }) => {
...
test.fixme('Do not include obscured layers', async () => {
  const url = await joinUrl(startParams, '&l=MODIS_Terra_CorrectedReflectance_TrueColor,MODIS_Terra_Aerosol,Reference_Features_15m')
  await page.goto(url)
  await closeModal(page)
  await openImageDownloadPanel(page)
...
test.fixme('Multiple base layers when one is semi-transparent', async () => {
