...
test('Re-ordering groups, then disabling groups keeps individual layer order', async () => {
  ...
  await page.goto(twoGroupsQueryString)
  await closeModal(page)
  const aodBoundingBox = await aodGroupHeader.boundingBox() || { x: 0, y: 0 }
  const firesBoundingBox = await firesGroupHeader.boundingBox() || { x: 0, y: 0, width: 0, height: 0 }
