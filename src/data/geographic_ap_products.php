<?php
	$json = array(
	"Dust Storms" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		 array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),	
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "MODIS Combined Value-Added Aerosol Optical Depth",
	        "sublabel" =>  "Terra and Aqua / MODIS",
	        "value" =>  "MODIS_Combined_Value_Added_AOD",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
        array(
        	"label" =>  "Dust Score",
        	"sublabel" =>  "Aqua / AIRS",
        	"value" =>  "AIRS_Dust_Score",
        	"type" => "multi", "category" => "overlays"
        ),
		array(
	        "label" =>  "Aerosol Index",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Index",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Absorbing Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Absorbing_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		)
	),


	"Fires" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),	
		 array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Fires (Day and Night)",
	        "sublabel" =>  "Terra/ and Aqua/MODIS Fire and Thermal Anomalies",
	        "value" =>  "MODIS_Fires_All",
	        "type" => "multi", "category" => "overlays", "tags" => "hotspot"
		),
		array(
	        "label" =>  "Fires (Day and Night)",
	        "sublabel" =>  "Terra/MODIS Fire and Thermal Anomalies",
	        "value" =>  "MODIS_Fires_Terra",
	        "type" => "multi", "category" => "overlays", "tags" => "hotspot"
		),
		array(
	        "label" =>  "Fires (Day and Night)",
	        "sublabel" =>  "Aqua/MODIS Fire and Thermal Anomalies",
	        "value" =>  "MODIS_Fires_Aqua",
	        "type" => "multi", "category" => "overlays", "tags" => "hotspot"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "MODIS Combined Value-Added Aerosol Optical Depth",
	        "sublabel" =>  "Terra and Aqua / MODIS",
	        "value" =>  "MODIS_Combined_Value_Added_AOD",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Land Surface Temperature (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Land_Surface_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Land_Surface_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Land_Surface_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Land_Surface_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Day",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Night",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
		array(
	        "label" =>  "Aerosol Index",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Index",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Absorbing Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Absorbing_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		)
	),


	"Drought" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Snow Cover",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Snow_Cover",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Snow Cover",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Snow_Cover",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Precipitation Estimate (Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Precipitation Estimate (Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays"	, "tags" => "borders, mask"	        
			),
        array(
	        "label" =>  "Drought Hazard: Frequency and Distribution",
	        "sublabel" =>  "SEDAC Global Drought Hazard Frequency and Distribution, v1 (1980–2000)",
	        "value" =>  "ndh-drought-hazard-frequency-distribution",
	        "type" => "multi", "category" => "overlays"
		),
        array(
	        "label" =>  "Drought Hazard: Mortality Risk",
	        "sublabel" =>  "SEDAC Global Drought Mortality Risks and Distribution, v1 (2000)",
	        "value" =>  "ndh-drought-mortality-risks-distribution",
	        "type" => "multi", "category" => "overlays"
		),
        array(
	        "label" =>  "Drought Hazard: Economic Risk",
	        "sublabel" =>  "SEDAC Global Drought Proportional Economic Loss Risk Deciles, v1 (2000)",
	        "value" =>  "ndh-drought-proportional-economic-loss-risk-deciles",
	        "type" => "multi", "category" => "overlays"
		)
	),


	"Smoke Plumes" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Corrected Reflectance (3-6-7)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands367",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr" 
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),	
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "MODIS Combined Value-Added Aerosol Optical Depth",
	        "sublabel" =>  "Terra and Aqua / MODIS",
	        "value" =>  "MODIS_Combined_Value_Added_AOD",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Day",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Night",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
        array(
            "label" =>  "Carbon Monoxide (215 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_CO_215hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "co"
        ),
        array(
            "label" =>  "Carbon Monoxide (215 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_CO_215hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "co"
        ),
		array(
	        "label" =>  "Aerosol Index",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Index",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Absorbing Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Absorbing_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays"	, "tags" => "borders, mask"	        
			)
	),

	"Ash Plumes" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),	
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "MODIS Combined Value-Added Aerosol Optical Depth",
	        "sublabel" =>  "Terra and Aqua / MODIS",
	        "value" =>  "MODIS_Combined_Value_Added_AOD",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Day, Prata Algorithm)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Prata_SO2_Index_Day",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Night, Prata Algorithm)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Prata_SO2_Index_Night",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Planetary Boundary Layer)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Planetary_Boundary_Layer",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Lower Troposphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Lower_Troposphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Middle Troposphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Middle_Troposphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Upper Troposphere and Stratosphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Upper_Troposphere_and_Stratosphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
        array(
            "label" =>  "Sulfur Dioxide (147 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_SO2_147hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "so2"
        ),
        array(
            "label" =>  "Sulfur Dioxide (147 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_SO2_147hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "so2"
        ),
        array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays"	, "tags" => "borders, mask"        
			),
        array(
             "label" =>  "Volcano Hazard =>  Frequency and Distribution",
             "sublabel" =>  "SEDAC Global Volcano Hazard Frequency and Distribution, v1 (1979–2000)",
             "value" =>  "ndh-volcano-hazard-frequency-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Volcano Hazard: Mortality Risk",
             "sublabel" =>  "SEDAC Global Volcano Mortality Risks and Distribution, v1 (2000)",
             "value" =>  "ndh-volcano-mortality-risks-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Volcano Hazard: Economic Risk",
             "sublabel" =>  "SEDAC Global Volcano Proportional Economic Loss Risk Deciles, v1 (2000)",
             "value" =>  "ndh-volcano-proportional-economic-loss-risk-deciles",
             "type" => "multi", "category" => "overlays"
        )
	),


	
	"Vegetation" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),	
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Snow Cover",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Snow_Cover",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Snow Cover",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Snow_Cover",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Land_Surface_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Land_Surface_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Land_Surface_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Land_Surface_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays"	, "tags" => "borders, mask"	        
		)
	
	),


	"Shipping" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),	
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Corrected Reflectance (3-6-7)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands367",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
	    array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Sea Ice",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Sea_Ice",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Sea Ice",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Sea_Ice",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Brightness_Temp_Band31_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Brightness_Temp_Band31_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Brightness_Temp_Band31_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Brightness_Temp_Band31_Night",
	        "type" => "multi", "category" => "overlays"
		),
		 array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays"	, "tags" => "borders, mask"        
		)
	),


	"Air Quality" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "MODIS Combined Value-Added Aerosol Optical Depth",
	        "sublabel" =>  "Terra and Aqua / MODIS",
	        "value" =>  "MODIS_Combined_Value_Added_AOD",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Day",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Night",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
        array(
            "label" =>  "Carbon Monoxide (215 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_CO_215hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "co"
        ),
        array(
            "label" =>  "Carbon Monoxide (215 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_CO_215hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "co"
        ),
		array(
	        "label" =>  "Sulfur Dioxide (Day, Prata Algorithm)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Prata_SO2_Index_Day",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Night, Prata Algorithm)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Prata_SO2_Index_Night",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
        array(
            "label" =>  "Sulfur Dioxide (147 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_SO2_147hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "so2"
        ),
        array(
            "label" =>  "Sulfur Dioxide (147 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_SO2_147hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "so2"
        ),
		array(
	        "label" =>  "Aerosol Index",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Index",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Absorbing Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Absorbing_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Planetary Boundary Layer)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Planetary_Boundary_Layer",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Lower Troposphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Lower_Troposphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Middle Troposphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Middle_Troposphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Upper Troposphere and Stratosphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Upper_Troposphere_and_Stratosphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays", "tags" => "borders, mask"        
		)
		
	),

	

	"Severe Storms" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Water Vapor (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Water_Vapor_5km_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Water Vapor (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Water_Vapor_5km_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Water Vapor (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Water_Vapor_5km_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Water Vapor (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Water_Vapor_5km_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Top Temperature (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Cloud_Top_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Top Temperature (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Cloud_Top_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Top Temperature (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Cloud_Top_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),		
		array(
	        "label" =>  "Cloud Top Temperature (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Cloud_Top_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),		
		array(
	        "label" =>  "Cloud Top Pressure (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Cloud_Top_Pressure_Day",
	        "type" => "multi", "category" => "overlays"
		),	
		array(
	        "label" =>  "Cloud Top Pressure (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Cloud_Top_Pressure_Night",
	        "type" => "multi", "category" => "overlays"
		),
        array(
            "label" =>  "Cloud Top Pressure (Day)",
            "sublabel" =>  "Aqua / MODIS",
            "value" =>  "MODIS_Aqua_Cloud_Top_Pressure_Day",
            "type" => "multi", "category" => "overlays"
        ),  
		array(
	        "label" =>  "Cloud Top Pressure (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Cloud_Top_Pressure_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Precipitation Estimate (Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Precipitation Estimate (Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Pressure",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Cloud_Pressure",
	        "type" => "multi", "category" => "overlays"
		),
		array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays", "tags" => "borders, mask"     
		),
        array(
             "label" =>  "Cyclone Hazard: Frequency and Distribution",
             "sublabel" =>  "SEDAC Global Cyclone Hazard Frequency and Distribution, v1 (1980–2000)",
             "value" =>  "ndh-cyclone-hazard-frequency-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Cyclone Hazard: Mortality Risk",
             "sublabel" =>  "SEDAC Global Cyclone Mortality Risks and Distribution, v1 (2000)",
             "value" =>  "ndh-cyclone-mortality-risks-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Cyclone Hazard: Economic Risk",
             "sublabel" =>  "SEDAC Global Cyclone Proportional Economic Loss Risk Deciles, v1 (2000)",
             "value" =>  "ndh-cyclone-proportional-economic-loss-risk-deciles",
             "type" => "multi", "category" => "overlays"
        ),
         
        array(
             "label" =>  "Flood Hazard: Frequency and Distribution",
             "sublabel" =>  "SEDAC Global Flood Hazard Frequency and Distribution, v1 (1985–2003)",
             "value" =>  "ndh-flood-hazard-frequency-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Flood Hazard: Mortality Risk",
             "sublabel" =>  "SEDAC Global Flood Mortality Risks and Distribution, v1 (2000)",
             "value" =>  "ndh-flood-mortality-risks-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Flood Hazard: Economic Risk",
             "sublabel" =>  "SEDAC Global Flood Proportional Economic Loss Risk Deciles, v1 (2000)",
             "value" =>  "ndh-flood-proportional-economic-loss-risk-deciles",
             "type" => "multi", "category" => "overlays"
        )

	),
	
	"Floods" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Precipitation Estimate (Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Precipitation Estimate (Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Night",
	        "type" => "multi", "category" => "overlays"
		),
        array(
            "label" => "Global 250m Water Map",
            "sublabel" => "Terra / MODIS, SRTM",
            "value" => "MODIS_Water_Mask",
            "type" => "multi", "category" => "overlays", "tags" => "mask"
        ),
        array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays", "tags" => "borders, mask"        
		),		
        array(
             "label" =>  "Flood Hazard: Frequency and Distribution",
             "sublabel" =>  "SEDAC Global Flood Hazard Frequency and Distribution, v1 (1985–2003)",
             "value" =>  "ndh-flood-hazard-frequency-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Flood Hazard: Mortality Risk",
             "sublabel" =>  "SEDAC Global Flood Mortality Risks and Distribution, v1 (2000)",
             "value" =>  "ndh-flood-mortality-risks-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Flood Hazard: Economic Risk",
             "sublabel" =>  "SEDAC Global Flood Proportional Economic Loss Risk Deciles, v1 (2000)",
             "value" =>  "ndh-flood-proportional-economic-loss-risk-deciles",
             "type" => "multi", "category" => "overlays"
        )
	),


	"All" =>  array(
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_TrueColor",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (True Color)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands143",
	        "type" => "single", "category" => "baselayers", "tags" => "natural color, lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (1-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands121",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Corrected Reflectance (3-6-7)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands367",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Corrected Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_CorrectedReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "cr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Land Surface Reflectance (7-2-1)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_SurfaceReflectance_Bands721",
	        "type" => "single", "category" => "baselayers", "tags" => "lsr"
		),
		array(
	        "label" =>  "Earth at Night 2012",
	        "sublabel" =>  "Suomi NPP / VIIRS via NASA Earth Observatory",
	        "value" =>  "VIIRS_CityLights_2012",
	        "type" => "single", "category" => "baselayers", "tags" => "night lights, city lights, snpp"
		),
		array(
		        "label"=> "Land / Water Map",
		        "sublabel"=> "OpenStreetMap / Coastlines",
		        "value"=> "land_water_map",
		        "type" => "single", "category" => "baselayers", "tags" => "borders"
			),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Aerosol",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "MODIS Combined Value-Added Aerosol Optical Depth",
	        "sublabel" =>  "Terra and Aqua / MODIS",
	        "value" =>  "MODIS_Combined_Value_Added_AOD",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Brightness_Temp_Band31_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Brightness_Temp_Band31_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Brightness_Temp_Band31_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Brightness Temperature (Band 31-Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Brightness_Temp_Band31_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Top Pressure (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Cloud_Top_Pressure_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Top Pressure (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Cloud_Top_Pressure_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Top Pressure (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Cloud_Top_Pressure_Day",
	        "type" => "multi", "category" => "overlays"
		),		
		array(
	        "label" =>  "Cloud Top Pressure (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Cloud_Top_Pressure_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Cloud Top Temperature (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Cloud_Top_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
        array(
            "label" =>  "Cloud Top Temperature (Night)",
            "sublabel" =>  "Terra / MODIS",
            "value" =>  "MODIS_Terra_Cloud_Top_Temp_Night",
            "type" => "multi", "category" => "overlays"
        ),
		array(
	        "label" =>  "Cloud Top Temperature (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Cloud_Top_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),		
		array(
	        "label" =>  "Cloud Top Temperature (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Cloud_Top_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),		
		array(
	        "label" =>  "Fires (Day and Night)",
	        "sublabel" =>  "Terra/ and Aqua/MODIS Fire and Thermal Anomalies",
	        "value" =>  "MODIS_Fires_All",
	        "type" => "multi", "category" => "overlays", "tags" => "hotspot"
		),
		array(
	        "label" =>  "Fires (Day and Night)",
	        "sublabel" =>  "Terra/MODIS Fire and Thermal Anomalies",
	        "value" =>  "MODIS_Fires_Terra",
	        "type" => "multi", "category" => "overlays", "tags" => "hotspot"
		),
		array(
	        "label" =>  "Fires (Day and Night)",
	        "sublabel" =>  "Aqua/MODIS Fire and Thermal Anomalies",
	        "value" =>  "MODIS_Fires_Aqua",
	        "type" => "multi", "category" => "overlays", "tags" => "hotspot"
		),
		array(
	        "label" =>  "Land Surface Temperature (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Land_Surface_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Land_Surface_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Land_Surface_Temp_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Land Surface Temperature (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Land_Surface_Temp_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Sea Ice",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Sea_Ice",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Sea Ice",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Sea_Ice",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Snow Cover",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Snow_Cover",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Snow Cover",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Snow_Cover",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Water Vapor (Day)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Water_Vapor_5km_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Water Vapor (Night)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Water_Vapor_5km_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Water Vapor (Day)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Water_Vapor_5km_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Water Vapor (Night)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Water_Vapor_5km_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Day",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
		array(
	        "label" =>  "Carbon Monoxide (Total Column, Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_CO_Total_Column_Night",
	        "type" => "multi", "category" => "overlays", "tags" => "co"
		),
        array(
        	"label" =>  "Dust Score",
        	"sublabel" =>  "Aqua / AIRS",
        	"value" =>  "AIRS_Dust_Score",
        	"type" => "multi", "category" => "overlays"
        ),
		array(
	        "label" =>  "Precipitation Estimate (Day)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Day",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Precipitation Estimate (Night)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Precipitation_Night",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Day, Prata Algorithm)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Prata_SO2_Index_Day",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Night, Prata Algorithm)",
	        "sublabel" =>  "Aqua / AIRS",
	        "value" =>  "AIRS_Prata_SO2_Index_Night",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Absorbing Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Absorbing_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Index",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Index",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Aerosol Optical Depth",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Aerosol_Optical_Depth",
	        "type" => "multi", "category" => "overlays", "tags" => "aod"
		),
		array(
	        "label" =>  "Cloud Pressure",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_Cloud_Pressure",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Planetary Boundary Layer)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Planetary_Boundary_Layer",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Lower Troposphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Lower_Troposphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Middle Troposphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Middle_Troposphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
		array(
	        "label" =>  "Sulfur Dioxide (Upper Troposphere and Stratosphere)",
	        "sublabel" =>  "Aura / OMI",
	        "value" =>  "OMI_SO2_Upper_Troposphere_and_Stratosphere",
	        "type" => "multi", "category" => "overlays", "tags" => "so2"
		),
        array(
            "label" =>  "Carbon Monoxide (215 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_CO_215hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "co"
        ),
        array(
            "label" =>  "Carbon Monoxide (215 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_CO_215hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "co"
        ),
        array(
            "label" =>  "Nitric Acid (46 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_HNO3_46hPa_Day",
            "type" => "multi", "category" => "overlays"
        ),
        array(
            "label" =>  "Nitric Acid (46 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_HNO3_46hPa_Night",
            "type" => "multi", "category" => "overlays"
        ),
        array(
            "label" =>  "Nitrous Oxide (46 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_N2O_46hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "n2o"
        ),
        array(
            "label" =>  "Nitrous Oxide (46 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_N2O_46hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "n2o"
        ),
        array(
            "label" =>  "Ozone (46 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_O3_46hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "o3"
        ),
        array(
            "label" =>  "Ozone (46 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_O3_46hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "o3"
        ),
        array(
            "label" =>  "Sulfur Dioxide (147 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_SO2_147hPa_Day",
            "type" => "multi", "category" => "overlays", "tags" => "so2"
        ),
        array(
            "label" =>  "Sulfur Dioxide (147 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_SO2_147hPa_Night",
            "type" => "multi", "category" => "overlays", "tags" => "so2"
        ),
        array(
            "label" =>  "Temperature (46 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_Temperature_46hPa_Day",
            "type" => "multi", "category" => "overlays"
        ),
        array(
            "label" =>  "Temperature (46 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_Temperature_46hPa_Night",
            "type" => "multi", "category" => "overlays"
        ),
        array(
            "label" =>  "Water Vapor (46 hPa, Day)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_H2O_46hPa_Day",
            "type" => "multi", "category" => "overlays"
        ),
        array(
            "label" =>  "Water Vapor (46 hPa, Night)",
            "sublabel" =>  "Aura / MLS",
            "value" =>  "MLS_H2O_46hPa_Night",
            "type" => "multi", "category" => "overlays"
        ),
        array(
		        "label"=> "Land Mask",
		        "sublabel"=> "OpenSteetMap / Coastlines",
		        "value"=> "land_mask",
		        "type" => "multi", "category" => "overlays", "tags" => "borders, mask"		        
			),
        array(
             "label" =>  "Cyclone Hazard:Frequency and Distribution",
             "sublabel" =>  "SEDAC Global Cyclone Hazard Frequency and Distribution, v1 (1980–2000)",
             "value" =>  "ndh-cyclone-hazard-frequency-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Cyclone Hazard: Mortality Risk",
             "sublabel" =>  "SEDAC Global Cyclone Mortality Risks and Distribution, v1 (2000)",
             "value" =>  "ndh-cyclone-mortality-risks-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Cyclone Hazard: Economic Risk",
             "sublabel" =>  "SEDAC Global Cyclone Proportional Economic Loss Risk Deciles, v1 (2000)",
             "value" =>  "ndh-cyclone-proportional-economic-loss-risk-deciles",
             "type" => "multi", "category" => "overlays"
        ),
        array(
	        "label" =>  "Drought Hazard: Frequency and Distribution",
	        "sublabel" =>  "SEDAC Global Drought Hazard Frequency and Distribution, v1 (1980–2000)",
	        "value" =>  "ndh-drought-hazard-frequency-distribution",
	        "type" => "multi", "category" => "overlays"
		),
        array(
	        "label" =>  "Drought Hazard: Mortality Risk",
	        "sublabel" =>  "SEDAC Global Drought Mortality Risks and Distribution, v1 (2000)",
	        "value" =>  "ndh-drought-mortality-risks-distribution",
	        "type" => "multi", "category" => "overlays"
		),
        array(
	        "label" =>  "Drought Hazard: Economic Risk",
	        "sublabel" =>  "SEDAC Global Drought Proportional Economic Loss Risk Deciles, v1 (2000)",
	        "value" =>  "ndh-drought-proportional-economic-loss-risk-deciles",
	        "type" => "multi", "category" => "overlays"
		),
        array(
             "label" =>  "Flood Hazard: Frequency and Distribution",
             "sublabel" =>  "SEDAC Global Flood Hazard Frequency and Distribution, v1 (1985–2003)",
             "value" =>  "ndh-flood-hazard-frequency-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Flood Hazard: Mortality Risk",
             "sublabel" =>  "SEDAC Global Flood Mortality Risks and Distribution, v1 (2000)",
             "value" =>  "ndh-flood-mortality-risks-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Flood Hazard: Economic Risk",
             "sublabel" =>  "SEDAC Global Flood Proportional Economic Loss Risk Deciles, v1 (2000)",
             "value" =>  "ndh-flood-proportional-economic-loss-risk-deciles",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Volcano Hazard: Frequency and Distribution",
             "sublabel" =>  "SEDAC Global Volcano Hazard Frequency and Distribution, v1 (1979–2000)",
             "value" =>  "ndh-volcano-hazard-frequency-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Volcano Hazard: Mortality Risk",
             "sublabel" =>  "SEDAC Global Volcano Mortality Risks and Distribution, v1 (2000)",
             "value" =>  "ndh-volcano-mortality-risks-distribution",
             "type" => "multi", "category" => "overlays"
        ),
        array(
             "label" =>  "Volcano Hazard: Economic Risk",
             "sublabel" =>  "SEDAC Global Volcano Proportional Economic Loss Risk Deciles, v1 (2000)",
             "value" =>  "ndh-volcano-proportional-economic-loss-risk-deciles",
             "type" => "multi", "category" => "overlays"
        ),
		array(
	        "label" =>  "Administrative Boundaries",
	        "sublabel" =>  "SEDAC / L1 Admin Boundaries",
	        "value" =>  "cartographic:esri-administrative-boundaries_level-1",
	        "type" => "multi", "category" => "overlays", "tags" => "state, borders"
		),
		array(
	        "label" =>  "Coastlines",
	        "sublabel" =>  "SEDAC / Coastlines v3",
	        "value" =>  "gpw-v3-coastlines",
	        "type" => "multi", "category" => "overlays", "tags" => "borders"
		),	
		array(
	        "label" =>  "Global Labels",
	        "sublabel" =>  "SEDAC / Global Labels",
	   		"value" =>  "cartographic:00-global-labels",
	        "type" => "multi", "category" => "overlays", "tags" => "names"
		),
		array(
	        "label" =>  "National Boundaries",
	        "sublabel" =>  "SEDAC / National Boundaries",
	        "value" =>  "sedac_bound",
	        "type" => "multi", "category" => "overlays", "tags" => "borders, countries"
		),
        array(
	        "label" =>  "Population Density",
	        "sublabel" =>  "SEDAC GRUMP v1 Population Count 2000",
	        "value" =>  "grump-v1-population-count_2000",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Latitude-Longitude Lines",
	        "sublabel" =>  "OpenLayers / Graticule Control",
	        "value" =>  "ol_graticule",
	        "type" => "multi", "category" => "overlays", "tags" => "lat lon, lat-lon"
		),
        array(
            "label" => "Global 250m Water Map",
            "sublabel" => "Terra / MODIS, SRTM",
            "value" => "MODIS_Water_Mask",
            "type" => "multi", "category" => "overlays", "tags" => "mask"
        ),   		
		array(
	        "label" =>  "Areas of No Data (mask)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Data_No_Data",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Areas of No Data (mask)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Data_No_Data",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Ascending)",
	        "sublabel" =>  "Space-Track.org / Terra",
	        "value" =>  "Terra_Orbit_Asc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Descending)",
	        "sublabel" =>  "Space-Track.org / Terra",
	        "value" =>  "Terra_Orbit_Dsc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Ascending)",
	        "sublabel" =>  "Space-Track.org / Aqua",
	        "value" =>  "Aqua_Orbit_Asc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Descending)",
	        "sublabel" =>  "Space-Track.org / Aqua",
	        "value" =>  "Aqua_Orbit_Dsc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Ascending)",
	        "sublabel" =>  "Space-Track.org / Aura",
	        "value" =>  "Aura_Orbit_Asc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Descending)",
	        "sublabel" =>  "Space-Track.org / Aura",
	        "value" =>  "Aura_Orbit_Dsc",
	        "type" => "multi", "category" => "overlays"
		)	
	),

	"Other" =>  array(
		array(
	        "label" =>  "Earth at Night 2012",
	        "sublabel" =>  "Suomi NPP / VIIRS via NASA Earth Observatory",
	        "value" =>  "VIIRS_CityLights_2012",
	        "type" => "single", "category" => "baselayers", "tags" => "night lights, city lights, snpp"
		),
        array(
	        "label" =>  "Population Density",
	        "sublabel" =>  "SEDAC GRUMP v1 Population Count 2000",
	        "value" =>  "grump-v1-population-count_2000",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Administrative Boundaries",
	        "sublabel" =>  "SEDAC / L1 Admin Boundaries",
	        "value" =>  "cartographic:esri-administrative-boundaries_level-1",
	        "type" => "multi", "category" => "overlays", "tags" => "state, borders"
		),
		array(
	        "label" =>  "National Boundaries",
	        "sublabel" =>  "SEDAC / National Boundaries",
	        "value" =>  "sedac_bound",
	        "type" => "multi", "category" => "overlays", "tags" => "borders, countries"
		),
		array(
	        "label" =>  "Coastlines",
	        "sublabel" =>  "SEDAC / Coastlines v3",
	        "value" =>  "gpw-v3-coastlines",
	        "type" => "multi", "category" => "overlays", "tags" => "borders"
		),	
		array(
	        "label" =>  "Global Labels",
	        "sublabel" =>  "SEDAC / Global Labels",
	   		"value" =>  "cartographic:00-global-labels",
	        "type" => "multi", "category" => "overlays", "tags" => "names"
		),
		array(
	        "label" =>  "Latitude-Longitude Lines",
	        "sublabel" =>  "OpenLayers / Graticule Control",
	        "value" =>  "ol_graticule",
	        "type" => "multi", "category" => "overlays", "tags" => "lat lon, lat-lon"
		),
        array(
            "label" => "Global 250m Water Map",
            "sublabel" => "Terra / MODIS, SRTM",
            "value" => "MODIS_Water_Mask",
            "type" => "multi", "category" => "overlays", "tags" => "mask"
        ),       		
		array(
	        "label" =>  "Areas of No Data (mask)",
	        "sublabel" =>  "Terra / MODIS",
	        "value" =>  "MODIS_Terra_Data_No_Data",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Areas of No Data (mask)",
	        "sublabel" =>  "Aqua / MODIS",
	        "value" =>  "MODIS_Aqua_Data_No_Data",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Ascending)",
	        "sublabel" =>  "Space-Track.org / Terra",
	        "value" =>  "Terra_Orbit_Asc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Descending)",
	        "sublabel" =>  "Space-Track.org / Terra",
	        "value" =>  "Terra_Orbit_Dsc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Ascending)",
	        "sublabel" =>  "Space-Track.org / Aqua",
	        "value" =>  "Aqua_Orbit_Asc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Descending)",
	        "sublabel" =>  "Space-Track.org / Aqua",
	        "value" =>  "Aqua_Orbit_Dsc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Ascending)",
	        "sublabel" =>  "Space-Track.org / Aura",
	        "value" =>  "Aura_Orbit_Asc",
	        "type" => "multi", "category" => "overlays"
		),
		array(
	        "label" =>  "Orbital Track (Descending)",
	        "sublabel" =>  "Space-Track.org / Aura",
	        "value" =>  "Aura_Orbit_Dsc",
	        "type" => "multi", "category" => "overlays"
		)
	));

	echo json_encode($json);
?>