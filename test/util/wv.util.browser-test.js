...
buster.testCase("wv.util.browser", {
    ...
    "Is Safari": function() {
        this.stub(wv.util.browser.tests, "navigator").returns({
    ...
    "CORS": function() {
        this.stub(wv.util.browser.tests, "safari").returns(true);
        this.stub(wv.util.browser.tests, "safariVersion").returns(7);
        buster.assert(wv.util.browser.tests.cors());
    },

    "Non-working CORS on Safari version <= 6": function() {
        this.stub(wv.util.browser.tests, "safari").returns(true);
        this.stub(wv.util.browser.tests, "safariVersion").returns(6);
        buster.refute(wv.util.browser.tests.cors());
    },

    "Web workers": function() {
        this.stub(wv.util.browser.tests, "window").returns({});
        buster.assert(wv.util.browser.tests.webWorkers());
    },

    "No web workers": function() {
        this.stub(wv.util.browser.tests, "window").returns(undefined);
    ...
    */
    ...
    "Small device": function() {
        this.stub(window, "$").returns({
            width: this.stub().returns(200),
            height: this.stub().returns(200)
        });
