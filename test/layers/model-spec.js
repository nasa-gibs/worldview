buster.testCase(
  ...
  (function() {
    ...
    self['Adds base layer'] = function() {
      ...
      buster.assert.calledWith(listener, _.find(l.active, { id: 'mask' }));
    ...
    self['Removes base layer'] = function() {
      ...
      buster.assert.calledWith(listener, def);
      buster.assert.called(changeListener);
    };
    ...
    self['Layer outside date range is not renderable'] = function() {
      ...
      console.log(models.date.selected);
      var list = l.get({
        renderable: true
      });
    ...
    self['Loads state'] = function() {
      ...
      l.load(state, errors);
      var terra_aod_obj = _.find(l.active, {
        id: 'terra-aod'
      });
      var terra_cr_obj = _.find(l.active, {
        id: 'terra-cr'
      });
      buster.assert(terra_aod_obj);
      buster.assert(terra_cr_obj);
    ...
    self['Loads state, opacity clamped at 1'] = function() {
      ...
      buster.assert.equals(1, opacity);
      buster.assert.equals(errors.length, 0);
    };
